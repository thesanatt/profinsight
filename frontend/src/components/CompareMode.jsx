...
function StatRow({ label, values, names, format, higherIsBetter = true }) {
  ...
  return (
    <div className="py-2.5" style={{ borderBottom: '1px solid var(--border)' }}>
      <div className="text-xs mb-1.5" style={{ color: 'var(--text-tertiary)' }}>{label}</div>
      <div className="flex gap-4">
        {values.map((v, i) => {
          const isBest = v === best && valid.filter(x => x === best).length === 1
          return (
            <div key={i} className="flex-1">
              <div className="text-lg font-bold" style={{ color: isBest ? 'var(--accent)' : 'var(--text-secondary)' }}>
                {v != null ? (format ? format(v) : v) : '—'}{isBest && <span className="text-xs ml-1">★</span>}
              </div>
              <div className="text-[10px] truncate" style={{ color: 'var(--text-tertiary)' }}>{names[i]}</div>
            </div>
          )
        })}
      </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>Compare professors</h2>
          <p className="text-sm mt-0.5" style={{ color: 'var(--text-tertiary)' }}>Select 2-4 professors</p>
        </div>
        ...
        {selected.map(id => {
          ...
          return (
            <span key={id} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm"
              ...
              {p.name}
              <button onClick={() => toggle(id)} className="ml-1" style={{ opacity: 0.6 }}>×</button>
            </span>
          )
        })}
        {selected.length < 4 && (
              ...
              <div className="absolute top-full left-0 mt-1 w-72 rounded-xl shadow-lg z-10 max-h-60 overflow-y-auto"
                ...
                {filtered.map(p => (
                  <button key={p.id} onClick={() => { toggle(p.id); setSearch('') }}
                    className="w-full text-left px-3 py-2 text-sm" style={{ borderBottom: '1px solid var(--border)' }}
                    onMouseEnter={e => e.target.style.background = 'var(--bg-card-hover)'}
                    onMouseLeave={e => e.target.style.background = 'transparent'}>
                    <span style={{ color: 'var(--text-primary)' }}>{p.name}</span>
                    <span className="ml-2 text-xs" style={{ color: 'var(--text-tertiary)' }}>{p.department}</span>
                  </button>
                ))}
              </div>
            )}
      ...
      {details.length >= 2 && (
        <div className="space-y-4">
          <div className="card p-5">
            <h3 className="font-semibold mb-3" style={{ color: 'var(--text-primary)' }}>Head to head</h3>
          ...
          {radarData.length > 0 && (
            <div className="card p-5">
              <h3 className="font-semibold mb-3" style={{ color: 'var(--text-primary)' }}>Sentiment comparison</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {details.map((d, i) => (
              <div key={d.professor_id} onClick={() => onSelect(d.professor_id)} className="card-hover p-4"
                ...
                <div className="text-xs mb-2" style={{ color: 'var(--text-tertiary)' }}>{d.department}</div>
                <div className="text-xs leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{d.verdict}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {details.length < 2 && selected.length < 2 && (
        <div className="text-center py-16 text-sm" style={{ color: 'var(--text-tertiary)' }}>Search and select at least 2 professors</div>
      )}
    </div>
  )
}
