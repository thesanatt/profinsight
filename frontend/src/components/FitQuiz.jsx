    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>Your best matches</h2>
            <p className="text-sm mt-0.5" style={{ color: 'var(--text-tertiary)' }}>{results.count} professors ranked by fit</p>
          </div>
          <div className="flex gap-3">
            <button onClick={() => setStep('quiz')} className="text-sm font-medium" style={{ color: 'var(--accent)' }}>Adjust</button>
            <button onClick={onClose} className="text-sm" style={{ color: 'var(--text-tertiary)' }}>Close</button>
          </div>
        </div>
          ...
          {results.results.map((p, i) => (
            <div key={p.id} onClick={() => onSelect(p.id)} className="card-hover px-5 py-3.5">
              <div className="flex items-center gap-4">
                <span className="text-lg font-bold w-6 text-right" style={{ color: 'var(--text-tertiary)' }}>#{i+1}</span>
                <FitRing score={p.fit_score} />
                <div className="flex-1 min-w-0">
                  <div className="font-semibold" style={{ color: 'var(--text-primary)' }}>{p.name}</div>
                  <div className="text-xs" style={{ color: 'var(--text-tertiary)' }}>{p.department}</div>
                  {p.fit_reasons?.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-1.5">
                      {p.fit_reasons.map((r, j) => (
                        <span key={j} className="text-[11px] px-2 py-0.5 rounded-md" style={{ background: 'var(--accent-bg)', color: 'var(--accent)' }}>{r}</span>
                      ))}
                    </div>
                  )}
                </div>
                <div className="hidden sm:flex items-center gap-4 text-sm">
                  ...
                  <div className="text-center">
                    <div className="font-bold" style={{ color: 'var(--text-secondary)' }}>{p.avg_difficulty?.toFixed(1)}</div>
                    <div className="text-[10px]" style={{ color: 'var(--text-tertiary)' }}>Difficulty</div>
                  </div>
                  <div className="text-xs" style={{ color: 'var(--text-tertiary)' }}>{p.num_ratings} reviews</div>
                </div>
              </div>
            </div>
          ))}
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>Find your perfect professor</h2>
          <p className="text-sm mt-0.5" style={{ color: 'var(--text-tertiary)' }}>Tell us how you learn and we'll match you</p>
        </div>
        <button onClick={onClose} className="text-sm" style={{ color: 'var(--text-tertiary)' }}>Close</button>
      </div>
      <div className="space-y-4">
        {questions.map(q => (
          <div key={q.key} className="card px-5 py-4">
            <div className="text-sm font-medium mb-3" style={{ color: 'var(--text-primary)' }}>{q.label}</div>
            <div className="flex items-center gap-3">
              <span className="text-xs w-24 text-right" style={{ color: 'var(--text-tertiary)' }}>{q.low}</span>
              ...
              <span className="text-xs w-24" style={{ color: 'var(--text-tertiary)' }}>{q.high}</span>
            </div>
          </div>
        ))}
        <div className="card px-5 py-4">
          <div className="text-sm font-medium mb-2" style={{ color: 'var(--text-primary)' }}>Department (optional)</div>
      ...
      <button onClick={submit} disabled={loading} className="btn-primary w-full mt-6 py-3 justify-center">
